...
# Questo LanguageEncoder sfrutta SentenceTransformer (di SBERT) mentre noi vogliamo usare CLIP base come sentance encoder.
# Quindi da modificare --> per questo ho creato la mia versione (più avanti la versione originale)
# === Versione Modificata di LanguageEncoder per usare CLIP base ===
import os
import pickle
...
class LanguageEncoder(nn.Module):
    """
    Flexible text encoder supporting both SentenceTransformer models and CLIP-based encoders.
    """
    ...
    def __init__(self, model_name: str,
        ...
        # Parte modificata per funzionare sia con CLIP che con SentenceTransformer (di BERT)
        if model_name.startswith("openai/clip") or "clip" in model_name.lower():
            # Use HuggingFace CLIPTextModel
            self.is_clip = True # Se il flag è true allora usa CLIP (da HuggingFace, CLIPTextModel e CLIPTokenizer)
            self.tokenizer = CLIPTokenizer.from_pretrained(model_name)
            self.model = CLIPTextModel.from_pretrained(model_name)
            self.hidden_size = self.model.config.hidden_size
            mask_ignore_token_ids = [self.tokenizer.pad_token_id]
            mask_token_id = self.tokenizer.mask_token_id or self.tokenizer.pad_token_id
        else:
            # Use SentenceTransformer
            self.is_clip = False # Se il flag è false allora usa SentenceTransformer (come faceva nel caso originale)
    ...
    def forward(self, x: List[str]):
        ...
        if self.is_clip:
            # Tokenize using HF CLIPTokenizer
            features = self.tokenizer(x, return_tensors="pt", padding=True, truncation=True)
            if self.training:
            ...
            if self.output_value == "sentence_embedding":
                # Use CLS token embedding for sentence embedding
                embeddings = outputs.last_hidden_state[:, 0, :]
            else:  # token_embeddings
                embeddings = outputs.last_hidden_state
                attn_mask = features["attention_mask"]
                # Convert to torch convention: True = not attended
                attn_mask = ~attn_mask.bool()
                return {"token_embeddings": embeddings, "attention_mask": attn_mask}
        else:
            # SentenceTransformer flow
            ...
            if self.output_value == "sentence_embedding":
                ...
                attn_mask = ~features["attention_mask"].bool()
                return {"token_embeddings": embeddings, "attention_mask": attn_mask}

        if self.output_value == "sentence_embedding" and self.normalize_embeddings:
